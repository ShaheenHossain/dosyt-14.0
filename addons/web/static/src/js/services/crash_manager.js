dosyt.define('web.ErrorDialogRegistry', function (require) {
"use strict";
...
dosyt.define('web.CrashManager', function (require) {
"use strict";
...
var CrashManager = AbstractService.extend({
    init: function () {
        ...
        this.dosytExceptionTitleMap = {
            'dosyt.addons.base.models.ir_mail_server.MailDeliveryException': _lt("MailDeliveryException"),
            'dosyt.exceptions.AccessDenied': _lt("Access Denied"),
            'dosyt.exceptions.AccessError': _lt("Access Error"),
            'dosyt.exceptions.MissingError': _lt("Missing Record"),
            'dosyt.exceptions.UserError': _lt("User Error"),
            'dosyt.exceptions.ValidationError': _lt("Validation Error"),
            'dosyt.exceptions.Warning': _lt("Warning"),
        };
    ...
    rpc_error: function(error) {
        ...
        // Dosyt custom exception: UserError, AccessError, ...
        if (_.has(this.dosytExceptionTitleMap, error.data.name)) {
            error = _.extend({}, error, {
                data: _.extend({}, error.data, {
                    message: error.data.arguments[0],
                    title: this.dosytExceptionTitleMap[error.data.name],
                }),
            });
    ...
    show_error: function (error) {
        ...
        var dialog = new dialogClass(this, {
            title: _.str.capitalize(error.type) || _t("Dosyt Error"),
        }, error);
...
var RedirectWarningHandler = Widget.extend(ExceptionHandler, {
    ...
    display: function() {
        ...
        new WarningDialog(this, {
            title: _.str.capitalize(error.type) || _t("Dosyt Warning"),
            buttons: [
...
core.crash_registry.add('dosyt.exceptions.RedirectWarning', RedirectWarningHandler);

function session_expired(cm) {
    return {
        display: function () {
            const notif = {
                type: _t("Dosyt Session Expired"),
                message: _t("Your Dosyt session expired. The current page is about to be refreshed."),
            };
            const options = {
                buttons: [{
                    text: _t("Ok"),
...
core.crash_registry.add('dosyt.http.SessionExpiredException', session_expired);
