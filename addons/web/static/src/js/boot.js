...
(function () {
    ...
    dosyt.testing = typeof QUnit === 'object';
    dosyt.remainingJobs = jobs;
    dosyt.__DEBUG__ = {
    ...
    dosyt.define = function () {
        var args = Array.prototype.slice.call(arguments);
        var name = typeof args[0] === 'string' ? args.shift() : ('__dosyt_job' + (jobUID++));
        ...
        if (jobs.length) {
            ...
            for (var k = 0; k < jobs.length; k++) {
                debugJobs[jobs[k].name] = job = {
                    ...
                    dependents: dosyt.__DEBUG__.getDependents(jobs[k].name),
                    name: jobs[k].name
                };
                if (jobs[k].error) {
                ...
                var deps = dosyt.__DEBUG__.getDependencies(job.name);
            ...
            missing = dosyt.__DEBUG__.getMissingJobs();
            failed = dosyt.__DEBUG__.getFailedJobs();
            ...
            if (dosyt.debug || failed.length || unloaded.length) {
                ...
                if (dosyt.debug && Object.keys(debugJobs).length) {
                    log('Debug:                   ', debugJobs);
                }
            }
        }
        dosyt.__DEBUG__.jsModules = {
            missing: missing,
    ...
    dosyt.processJobs = function (jobs, services) {
        var job;
                ...
                if (!job.error) {
                    Promise.resolve(jobExec).then(
                        function (data) {
                            ...
                            resolve();
                            dosyt.processJobs(jobs, services);
