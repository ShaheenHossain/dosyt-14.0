...
from dosyt import http, _
from dosyt.addons.hw_drivers.controllers.proxy import proxy_drivers
from dosyt.addons.hw_drivers.driver import Driver
from dosyt.addons.hw_drivers.event_manager import event_manager
...
class KeyboardUSBDriver(Driver):
        ...
        Args:
            new_layout (dict): A dict containing two keys:
                ...
                - variant (str): An optional key to represent the variant of the
                                 selected layout
        """
        file_path = Path.home() / 'dosyt-keyboard-layouts.conf'
        if file_path.exists():
            data = json.loads(file_path.read_text())
        else:
            data = {}
        data[self.device_identifier] = layout
        helpers.write_file('dosyt-keyboard-layouts.conf', json.dumps(data))
    ...
    def save_is_scanner(self, is_scanner):
        ...
        """
        file_path = Path.home() / 'dosyt-keyboard-is-scanner.conf'
        if file_path.exists():
            data = json.loads(file_path.read_text())
        else:
            data = {}
        data[self.device_identifier] = is_scanner
        helpers.write_file('dosyt-keyboard-is-scanner.conf', json.dumps(data))
