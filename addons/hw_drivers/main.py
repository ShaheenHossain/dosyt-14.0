...
class Manager(Thread):
    def send_alldevices(self):
        """
        This method send IoT Box and devices informations to Dosyt database
        """
        server = helpers.get_dosyt_server_url()
        if server:
            subject = helpers.read_file_first_line('dosyt-subject.conf')
            if subject:
            ...
            _logger.warning('Dosyt server not set')

    def run(self):
        """
        Thread that will load interfaces and drivers and contact the dosyt server with the updates
        """
