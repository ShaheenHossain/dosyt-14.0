...
from dosyt.addons.mass_mailing.tests.common import MassMailCommon
from dosyt.tests.common import users, Form, tagged
from dosyt.tools import formataddr, mute_logger
...
class TestMassMailFeatures(MassMailCommon):
    ...
    def test_mailing_shortener(self):
        mailing = self.env['mailing.mailing'].create({
            ...
            'body_html': """<div>
Hi,
% set url = "www.dosyt.com"
% set httpurl = "https://www.dosyt.eu"
Website0: <a id="url0" href="https://www.dosyt.tz/my/${object.name}">https://www.dosyt.tz/my/${object.name}</a>
Website1: <a id="url1" href="https://www.dosyt.be">https://www.dosyt.be</a>
...
Email: <a id="url5" href="mailto:test@dosyt.com">test@dosyt.com</a></div>""",
            'mailing_model_id': self.env['ir.model']._get('mailing.list').id,
            'reply_to_mode': 'email',
            'reply_to': self.email_reply_to,
            'contact_list_ids': [(6, 0, self.mailing_list_1.ids)],
        ...
        for contact in self.mailing_list_1.contact_ids:
            ...
            for link_info in [('url0', 'https://www.dosyt.tz/my/%s' % contact.name, True),
                              ('url1', 'https://www.dosyt.be', True),
                              ('url2', 'https://www.dosyt.com', True),
                              ('url3', 'https://www.dosyt.eu', True),
                              ('url4', 'https://www.example.com/foo/bar?baz=qux', True),
                              ('url5', 'mailto:test@dosyt.com', False)]:
