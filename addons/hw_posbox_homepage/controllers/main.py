...
from dosyt.http import Response
from dosyt.modules.module import get_resource_path

from dosyt.addons.hw_drivers.main import iot_devices
from dosyt.addons.hw_drivers.tools import helpers
from dosyt.addons.web.controllers import main as web
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('dosyt.addons.hw_posbox_homepage', "views")
...
class IoTboxHomepage(web.Home):
    ...
    def clean_partition(self):
        subprocess.check_call(['sudo', 'bash', '-c', '. /home/pi/dosyt/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; cleanup'])

    def get_six_terminal(self):
        terminal_id = helpers.read_file_first_line('dosyt-six-payment-terminal.conf')
    ...
    def get_homepage_data(self):
        ...
        return {
            ...
            'server_status': helpers.get_dosyt_server_url() or 'Not Configured',
    ...
    def index(self):
        ...
        remote_server = Path.home() / 'dosyt-remote-server.conf'
    ...
    def list_handlers(self):
        ...
        return handler_list_template.render({
            ...
            'server': helpers.get_dosyt_server_url()
        })
    ...
    def load_iot_handlers(self):
        ...
        subprocess.check_call(["sudo", "service", "dosyt", "restart"])
    ...
    def list_credential(self):
        return list_credential_template.render({
            ...
            'db_uuid': helpers.read_file_first_line('dosyt-db-uuid.conf'),
            'enterprise_code': helpers.read_file_first_line('dosyt-enterprise-code.conf'),
        })
    ...
    def clear_credential(self):
        helpers.unlink_file('dosyt-db-uuid.conf')
        helpers.unlink_file('dosyt-enterprise-code.conf')
        subprocess.check_call(["sudo", "service", "dosyt", "restart"])
        return "<meta http-equiv='refresh' content='20; url=http://" + helpers.get_ip() + ":8069'>"
    ...
    def clear_server_configuration(self):
        helpers.unlink_file('dosyt-remote-server.conf')
        return "<meta http-equiv='refresh' content='0; url=http://" + helpers.get_ip() + ":8069'>"
    ...
    def connect_to_server(self, token, iotname):
        if token:
            ...
            url = helpers.get_dosyt_server_url()
    ...
    def step_by_step_configure_page(self):
        return configure_wizard_template.render({
            ...
            'ssid': helpers.get_wifi_essid(),
            'server': helpers.get_dosyt_server_url() or '',
    ...
    def server(self):
        return server_config_template.render({
            ...
            'hostname': subprocess.check_output('hostname').decode('utf-8').strip('\n'),
            'server_status': helpers.get_dosyt_server_url() or 'Not configured yet',
    ...
    def add_six_payment_terminal(self, terminal_id):
        helpers.write_file('dosyt-six-payment-terminal.conf', terminal_id)
        subprocess.check_call(["sudo", "service", "dosyt", "restart"])
        return 'http://' + helpers.get_ip() + ':8069'

    @http.route('/six_payment_terminal_clear', type='http', auth='none', cors='*', csrf=False)
    def clear_six_payment_terminal(self):
        helpers.unlink_file('dosyt-six-payment-terminal.conf')
        subprocess.check_call(["sudo", "service", "dosyt", "restart"])
        return "<meta http-equiv='refresh' content='0; url=http://" + helpers.get_ip() + ":8069'>"

    @http.route('/hw_proxy/upgrade', type='http', auth='none', )
    def upgrade(self):
        commit = subprocess.check_output(["git", "--work-tree=/home/pi/dosyt/", "--git-dir=/home/pi/dosyt/.git", "log", "-1"]).decode('utf-8').replace("\n", "<br/>")
    ...
    def perform_upgrade(self):
        ...
        os.system('/home/pi/dosyt/addons/point_of_sale/tools/posbox/configuration/posbox_update.sh')
    ...
    def perform_flashing_create_partition(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/dosyt/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; create_partition']).decode().split('\n')[-2]
    ...
    def perform_flashing_download_raspios(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/dosyt/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; download_raspios']).decode().split('\n')[-2]
    ...
    def perform_flashing_copy_raspios(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/dosyt/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; copy_raspios']).decode().split('\n')[-2]
