dosyt.define('partner.autocomplete.Mixin', function (require) {
'use strict';
...
var PartnerAutocompleteMixin = {
    ...
    _autocomplete: function (value) {
        ...
        var dosytSuggestions = [];
        var clearbitSuggestions = [];
        return new Promise(function (resolve, reject) {
            var dosytPromise = self._getDosytSuggestions(value, isVAT).then(function (suggestions){
                dosytSuggestions = suggestions;
            });
            ...
            var concatResults = function () {
                ...
                if (clearbitSuggestions && clearbitSuggestions.length) {
                    var websites = dosytSuggestions.map(function (suggestion) {
                    ...
                    clearbitSuggestions.forEach(function (suggestion) {
                        if (websites.indexOf(suggestion.domain) < 0) {
                            ...
                            dosytSuggestions.push(suggestion);
                        }
                    });
                }

                dosytSuggestions = _.filter(dosytSuggestions, function (suggestion) {
                    return !suggestion.ignored;
                });
                _.each(dosytSuggestions, function(suggestion){
                delete suggestion.ignored;
                });
                return resolve(dosytSuggestions);
            };

            self._whenAll([dosytPromise, clearbitPromise]).then(concatResults, concatResults);
        });
