...
from dosyt import _, models
from dosyt.tools import float_repr
from dosyt.tests.common import Form
...
class AccountEdiCommon(models.AbstractModel):
    ...
    def _import_fill_invoice_line_values(self, tree, xpath_dict, invoice_line_form, qty_factor):
        """
        Read the xml invoice, extract the invoice line values, compute the dosyt values
        ...
        if quantity_node is not None:
            ...
            if uom_xml:
                uom_infered_xmlid = [
                    dosyt_xmlid for dosyt_xmlid, uom_unece in UOM_TO_UNECE_CODE.items() if uom_unece == uom_xml
                ]
