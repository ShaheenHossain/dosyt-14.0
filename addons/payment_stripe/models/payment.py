...
from dosyt.tools.float_utils import float_round
from dosyt.tools import consteq
from dosyt.exceptions import ValidationError

from dosyt.addons.payment_stripe.controllers.main import StripeController
...
class PaymentAcquirerStripe(models.Model):
    ...
    def _handle_checkout_webhook(self, checkout_object: dir):
        ...
        try:
            dosyt_tx = self.env['payment.transaction']._stripe_form_get_tx_from_data(data)
        except ValidationError as e:
            _logger.info('Received notification for tx %s. Skipped it because of %s', tx_reference, e)
            return False

        PaymentAcquirerStripe._verify_stripe_signature(dosyt_tx.acquirer_id)

        url = 'payment_intents/%s' % dosyt_tx.stripe_payment_intent
        stripe_tx = dosyt_tx.acquirer_id._stripe_request(url)
