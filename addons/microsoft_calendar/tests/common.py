...
from dosyt.tests.common import HttpCase

from dosyt.addons.microsoft_calendar.models.microsoft_sync import MicrosoftSync
...
class TestCommon(HttpCase):
    ...
    def setUp(self):
        ...
        self.expected_dosyt_event_from_outlook = {
            "name": "simple_event",
            "description": "my simple event",
            "active": True,
            "start": self.start_date,
        ...
        self.expected_dosyt_recurrency_from_outlook = {
            'active': True,
            'byday': '1',
            'count': 0,
            'day': 0,
        ...
        self.expected_dosyt_recurrency_events_from_outlook = [
            {
    ...
    def assert_dosyt_event(self, dosyt_event, expected_values):
        """
        ...
        dosyt_event_values = dosyt_event.read(list(expected_values.keys()))[0]
        for k, v in expected_values.items():
            if k in ("user_id", "recurrence_id"):
                v = (v.id, v.name) if v else False

            if isinstance(v, list):
                self.assertListEqual(sorted(v), sorted(dosyt_event_values.get(k)), msg=f"'{k}' mismatch")
            else:
                self.assertEqual(v, dosyt_event_values.get(k), msg=f"'{k}' mismatch")

    def assert_dosyt_recurrence(self, dosyt_recurrence, expected_values):
        """
        Assert that an Dosyt recurrence has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        dosyt_recurrence_values = dosyt_recurrence.read(list(expected_values.keys()))[0]

        for k, v in expected_values.items():
            self.assertEqual(v, dosyt_recurrence_values.get(k), msg=f"'{k}' mismatch")
