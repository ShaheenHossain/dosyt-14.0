...
class MicrosoftEvent(abc.Set):
    ...
    def _meta_dosyt_id(self, microsoft_guid):
    ...
    def dosyt_ids(self):
        ...
        """
        return tuple(e._dosyt_id for e in self if e._dosyt_id)

    def _load_dosyt_ids_from_db(self, env, force_model=None):
        """
        ...
        """
        mapped_events = [e.id for e in self if e._dosyt_id]
        ...
        dosyt_events = model_env.with_context(active_test=False).search([
            '|',
        ...
        dosyt_events_with_uids = dosyt_events.filtered(lambda e: e.ms_universal_event_id)
        mapping = {e.ms_universal_event_id: e.id for e in dosyt_events_with_uids}

        for ms_event in unmapped_events_with_uids:
            dosyt_id = mapping.get(ms_event.iCalUId)
            if dosyt_id:
                ms_event._events[ms_event.id]['_dosyt_id'] = dosyt_id
                mapped_events.append(ms_event.id)

        # 2. try to match unmapped events with Dosyt events using their id
        unmapped_events = self.filter(lambda e: e.id not in mapped_events)
        mapping = {e.ms_organizer_event_id: e for e in dosyt_events}

        for ms_event in unmapped_events:
            dosyt_event = mapping.get(ms_event.id)
            if dosyt_event:
                ms_event._events[ms_event.id]['_dosyt_id'] = dosyt_event.id
                mapped_events.append(ms_event.id)
    ...
    def match_with_dosyt_events(self, env) -> 'MicrosoftEvent':
        """
        ...
        mapped_recurrences = recurrence_candidates._load_dosyt_ids_from_db(env, force_model=env["calendar.recurrence"])

        # then, try to match events
        events_candidates = (self - mapped_recurrences).filter(lambda x: not x.is_recurrence())
        mapped_events = events_candidates._load_dosyt_ids_from_db(env)
