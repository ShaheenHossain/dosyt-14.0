...
echo "addons/point_of_sale/tools/posbox/overwrite_after_init/home/pi/dosyt" >> .git/info/sparse-checkout

git fetch "${localremote}" "${localbranch}" --depth=1
git reset "${localremote}"/"${localbranch}" --hard

git clean -dfx
cp -a /home/pi/dosyt/addons/point_of_sale/tools/posbox/overwrite_after_init/home/pi/dosyt/* /home/pi/dosyt/
rm -r /home/pi/dosyt/addons/point_of_sale/tools/posbox/overwrite_after_init
...
(sleep 5 && sudo service dosyt restart) &
