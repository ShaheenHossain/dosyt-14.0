...
copy_raspios () {
    ...
    wget -c 'https://nightly.dosyt.com/master/iotbox/iotbox-latest.zip' -O raspios/iotbox-latest.zip
    wget -c 'https://nightly.dosyt.com/master/iotbox/SHA1SUMS.txt' -O raspios/SHA1SUMS.txt
    cd raspios/
...
copy_iot () {
    ...
    # Modify startup
    mkdir -v dosyt
    mount -v "${PART_IOTBOX_ROOT}" dosyt
    cp -v /home/pi/upgrade.sh dosyt/home/pi/
    NBR_LIGNE=$(sed -n -e '$=' dosyt/etc/rc.local)
    sed -ie "${NBR_LIGNE}"'i\. /home/pi/upgrade.sh; clean_local' dosyt/etc/rc.local
    find /home/pi/config -maxdepth 1 -type f ! -name ".*" -exec cp {} dosyt/home/pi/ \;
