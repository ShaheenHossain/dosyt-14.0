...
REPO=https://github.com/dosyt/dosyt.git
...
CLONE_DIR="${OVERWRITE_FILES_BEFORE_INIT_DIR}/home/pi/dosyt"
...
dosyt/
dosyt-bin" | tee --append .git/info/sparse-checkout > /dev/null
...
# copy iotbox version
mkdir -pv "${MOUNT_POINT}"/var/dosyt
echo "${VERSION_IOTBOX}" | tee "${MOUNT_POINT}"/var/dosyt/iotbox_version "${MOUNT_POINT}"/home/pi/iotbox_version
