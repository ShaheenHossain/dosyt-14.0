dosyt.define('mail_bot/static/src/models/messaging_initializer/messaging_initializer_tests.js', function (require) {
"use strict";
...
QUnit.test('DosytBot initialized at init', async function (assert) {
    ...
    await this.start({
        env: {
            session: {
                dosytbot_initialized: false,
            },
        },
        async mockRPC(route, args) {
            if (args.method === 'init_dosytbot') {
                assert.step('init_dosytbot');
            }
