...
from dosyt.addons.payment.tests.common import PaymentAcquirerCommon
from dosyt.addons.payment_authorize.controllers.main import AuthorizeController
...
@dosyt.tests.tagged('post_install', '-at_install', '-standard', 'external')
class AuthorizeForm(AuthorizeCommon):
    ...
    @mute_logger('dosyt.addons.payment_authorize.models.payment', 'ValidationError')
...
@dosyt.tests.tagged('post_install', '-at_install', '-standard')
class AuthorizeS2s(AuthorizeCommon):
    def test_30_authorize_s2s(self):
        ...
        # add credential
        # FIXME: put this test in master-nightly on dosyt/dosyt + create sandbox account
        authorize.write({
