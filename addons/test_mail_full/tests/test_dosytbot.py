...
from dosyt.addons.test_mail.tests.common import TestMailCommon, TestRecipients
...
class TestDosytbot(TestMailCommon, TestRecipients):
    ...
    def setUpClass(cls):
        ...
        cls.dosytbot = cls.env.ref("base.partner_root")
        ...
        cls.dosytbot_ping_body = '<a href="http://dosyt.com/web#model=res.partner&amp;id=%s" class="o_mail_redirect" data-oe-id="%s" data-oe-model="res.partner" target="_blank">@DosytBot</a>' % (cls.dosytbot.id, cls.dosytbot.id)
    ...
    def test_fetch_listener(self):
        channel = self.env['mail.channel'].with_user(self.user_employee).init_dosytbot()
        partners = self.env['mail.channel'].channel_fetch_listeners(channel.uuid)
        dosytbot = self.env.ref("base.partner_root")
        dosytbot_in_fetch_listeners = [partner for partner in partners if partner['id'] == dosytbot.id]
        self.assertEqual(len(dosytbot_in_fetch_listeners), 1, 'dosytbot should appear only once in channel_fetch_listeners')

    @mute_logger('dosyt.addons.mail.models.mail_mail')
    def test_dosytbot_ping(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': self.dosytbot_ping_body, 'partner_ids': [self.dosytbot.id, self.user_admin.partner_id.id]})
        ...
        self.assertNotIn(self.dosytbot, follower)
    ...
    def test_onboarding_flow(self):
        ...
        self.assertNextMessage(
            last_message,  # no message will be post with command help, use last dosytbot message instead
            sender=self.dosytbot,
            answer=("@DosytBot",)
        )
        kwargs['body'] = ''
        ...
        channel.message_post(**kwargs)
        self.assertNotEqual(self.user_employee.dosytbot_state, 'onboarding_attachement')

        # Test miscellaneous messages
        self.user_employee.dosytbot_state = "idle"
        kwargs['partner_ids'] = []
    ...
    @mute_logger('dosyt.addons.mail.models.mail_mail')
    def test_dosytbot_no_default_answer(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': "I'm not talking to @dosytbot right now", 'partner_ids': []})
        self.assertNextMessage(
