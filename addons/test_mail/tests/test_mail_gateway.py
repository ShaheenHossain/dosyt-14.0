...
from dosyt import exceptions
from dosyt.addons.mail.tests.common import mail_new_test_user
from dosyt.addons.test_mail.data import test_mail_data
from dosyt.addons.test_mail.data.test_mail_data import MAIL_TEMPLATE
from dosyt.addons.test_mail.models.test_mail_models import MailTestGateway
from dosyt.addons.test_mail.tests.common import TestMailCommon
from dosyt.tests import tagged
from dosyt.tests.common import users
from dosyt.tools import email_split_and_format, formataddr, mute_logger
...
class TestEmailParsing(TestMailCommon):
    ...
    def test_message_parse_and_replace_binary_octetstream(self):
        ...
        with self.assertLogs('dosyt.addons.mail.models.mail_thread', level="WARNING") as capture:
        ...
        self.assertEqual(capture.output, [
            ("WARNING:dosyt.addons.mail.models.mail_thread:Message containing an unexpected "
    ...
    def test_message_parse_extract_bounce_rfc822_headers_qp(self):
        ...
        incoming_bounce = self.format(
            ...
            email_from='MAILER-DAEMON@mailserver.dosyt.com (Mail Delivery System)',
            email_to='bounce@xxx.dosyt.com',
            delivered_to='bounce@xxx.dosyt.com'
        )
...
class TestMailgateway(TestMailCommon):
    ...
    @mute_logger('dosyt.addons.mail.models.mail_thread', 'dosyt.models')
    ...
    @mute_logger('dosyt.addons.mail.models.mail_thread')
    ...
    def test_message_process_followers(self):
        """ Incoming email: recognized author not archived and not dosytbot: added as follower """
        ...
        dosytbot = self.env.ref('base.partner_root')
        dosytbot.active = True
        dosytbot.email = 'dosytbot@example.com'
        with self.mock_mail_gateway():
            record4 = self.format_and_process(
                MAIL_TEMPLATE, dosytbot.email_formatted, 'groups@test.com',
                subject='Dosytbot Automatic Answer')

        self.assertEqual(record4.message_ids[0].author_id, dosytbot)
        self.assertEqual(record4.message_ids[0].email_from, dosytbot.email_formatted)
    ...
    @mute_logger('dosyt.addons.mail.models.mail_thread', 'dosyt.models.unlink', 'dosyt.addons.mail.models.mail_mail')
    ...
    @mute_logger('dosyt.addons.mail.models.mail_thread', 'dosyt.addons.mail.models.mail_mail', 'dosyt.models.unlink')
    ...
    def test_message_process_references_external(self):
        """ Incoming email being a reply to an external email processed by dosyt should update thread accordingly """
    ...
    def test_message_process_references_external_buggy_message_id(self):
        ...
        dosyt should update thread accordingly. Special case when the
