...
import dosyt
from dosyt import api, models, registry, exceptions, tools, http
from dosyt.addons.base.models.ir_http import RequestUID, ModelConverter
from dosyt.addons.base.models.qweb import QWebException
from dosyt.http import request
from dosyt.osv import expression
from dosyt.tools import config, ustr, pycompat
...
class IrHttp(models.AbstractModel):
    ...
    def _geoip_setup_resolver(cls):
        ...
        if dosyt._geoip_resolver is not None:
            return
        geofile = config.get('geoip_database')
        try:
            dosyt._geoip_resolver = GeoIPResolver.open(geofile) or False
    ...
    def _geoip_resolve(cls):
        if 'geoip' not in request.session:
            record = {}
            if dosyt._geoip_resolver and request.httprequest.remote_addr:
                record = dosyt._geoip_resolver.resolve(request.httprequest.remote_addr) or {}
    ...
    def _postprocess_args(cls, arguments, rule):
        ...
        except dosyt.exceptions.MissingError:
