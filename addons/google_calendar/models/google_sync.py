...
from dosyt import api, fields, models, registry, _
from dosyt.tools import ormcache_context
from dosyt.exceptions import UserError
from dosyt.osv import expression

from dosyt.addons.google_calendar.utils.google_event import GoogleEvent
from dosyt.addons.google_calendar.utils.google_calendar import GoogleCalendarService
from dosyt.addons.google_account.models.google_service import TIMEOUT

_logger = logging.getLogger(__name__)


# API requests are sent to Google Calendar after the current transaction ends.
# This ensures changes are sent to Google only if they really happened in the Dosyt database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Google if the first creation crashed in Dosyt.
...
class GoogleSync(models.AbstractModel):
    ...
    def unlink(self):
        ...
        if self.env.context.get('archive_on_error') and self._active_name:
            ...
            # Then, since it has been deleted on Google, the event is also deleted on Dosyt DB (_sync_google2dosyt).
            self.action_archive()
            return True
        return super().unlink()
    ...
    def _sync_dosyt2google(self, google_service: GoogleCalendarService):
        if not self:
            return
    ...
    @api.model
    def _sync_google2dosyt(self, google_events: GoogleEvent, default_reminders=()):
        """Synchronize Google recurrences in Dosyt. Creates new recurrences, updates
        existing ones.

        :param google_recurrences: Google recurrences to synchronize in Dosyt
        :return: synchronized dosyt recurrences
        """
        existing = google_events.exists(self.env)
        new = google_events - existing - google_events.cancelled()

        dosyt_values = [
            dict(self._dosyt_values(e, default_reminders), need_sync=False)
            for e in new
        ]
        new_dosyt = self.with_context(dont_notify=True)._create_from_google(new, dosyt_values)
        ...
        if not self._context.get("dont_notify") and all(not e.is_recurrence() for e in google_events):
            new_dosyt._notify_attendees()

        cancelled = existing.cancelled()
        cancelled_dosyt = self.browse(cancelled.dosyt_ids(self.env))
        cancelled_dosyt._cancel()
        synced_records = (new_dosyt + cancelled_dosyt).with_context(dont_notify=self._context.get("dont_notify", False))
        for gevent in existing - cancelled:
            # Last updated wins.
            # This could be dangerous if google server time and dosyt server time are different
            updated = parse(gevent.updated)
            dosyt_record = self.browse(gevent.dosyt_id(self.env))
            # Migration from 13.4 does not fill write_date. Therefore, we force the update from Google.
            if not dosyt_record.write_date or updated >= pytz.utc.localize(dosyt_record.write_date):
                vals = dict(self._dosyt_values(gevent, default_reminders), need_sync=False)
                dosyt_record._write_from_google(gevent, vals)
                synced_records |= dosyt_record
    ...
    def _get_records_to_sync(self, full_sync=False):
        """Return records that should be synced from Dosyt to Google
    ...
    @api.model
    def _dosyt_values(self, google_event: GoogleEvent, default_reminders=()):
    ...
    def _notify_attendees(self):
        ...
        This is called when creating new calendar events in _sync_google2dosyt.
        At the initialization of a synced calendar, Dosyt requests all events for a specific
