...
class RecurrenceRule(models.Model):
    ...
    def _write_from_google(self, gevent, vals):
        ...
        new_event_values = self.env["calendar.event"]._dosyt_values(gevent)
    ...
    def _create_from_google(self, gevents, vals_list):
        for gevent, vals in zip(gevents, vals_list):
            base_values = dict(
                self.env['calendar.event']._dosyt_values(gevent),  # FIXME default reminders
                need_sync=False,
            )
    ...
    @api.model
    def _dosyt_values(self, google_recurrence, default_reminders=()):
        return {
