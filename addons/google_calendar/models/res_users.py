...
import logging
import requests
from dosyt.addons.google_calendar.models.google_sync import google_calendar_token
from datetime import timedelta


from dosyt import api, fields, models, _
from dosyt.exceptions import UserError
from dosyt.loglevels import exception_to_unicode
from dosyt.addons.google_account.models.google_service import GOOGLE_TOKEN_ENDPOINT
from dosyt.addons.google_calendar.utils.google_calendar import GoogleCalendarService, InvalidSyncToken
...
class User(models.Model):
    ...
    def _sync_google_calendar(self, calendar_service: GoogleCalendarService):
        ...
        synced_recurrences = self.env['calendar.recurrence']._sync_google2dosyt(recurrences)
        synced_events = self.env['calendar.event']._sync_google2dosyt(events - recurrences, default_reminders=default_reminders)

        # Dosyt -> Google
        send_updates = not full_sync
        recurrences = self.env['calendar.recurrence']._get_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences.with_context(send_updates=send_updates)._sync_dosyt2google(calendar_service)
        ...
        (events - synced_events).with_context(send_updates=send_updates)._sync_dosyt2google(calendar_service)
