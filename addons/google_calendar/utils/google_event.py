...
from dosyt.api import model
from dosyt.tools.sql import existing_tables
import pytz
import logging
...
class GoogleEvent(abc.Set):
    ...
    def dosyt_id(self, env):
        self.dosyt_ids(env)  # load ids
        return self._dosyt_id

    def _meta_dosyt_id(self, dbname):
        ...
        o_id = properties.get('%s_dosyt_id' % dbname)
        if o_id:
            return int(o_id)

    def dosyt_ids(self, env):
        ids = tuple(e._dosyt_id for e in self if e._dosyt_id)
        if len(ids) == len(self):
            return ids
        model = self._get_model(env)
        found = self._load_dosyt_ids_from_db(env, model)
        unsure = self - found
        if unsure:
            unsure._load_dosyt_ids_from_metadata(env, model)

        return tuple(e._dosyt_id for e in self)

    def _load_dosyt_ids_from_metadata(self, env, model):
        unsure_dosyt_ids = tuple(e._meta_dosyt_id(env.cr.dbname) for e in self)
        dosyt_events = model.browse(_id for _id in unsure_dosyt_ids if _id)
        ...
        # to match events.
        o_ids = dosyt_events.exists().filtered(lambda e: not e.google_id).ids
        for e in self:
            dosyt_id = e._meta_dosyt_id(env.cr.dbname)
            if dosyt_id in o_ids:
                e._events[e.id]['_dosyt_id'] = dosyt_id

    def _load_dosyt_ids_from_db(self, env, model):
        dosyt_events = model.with_context(active_test=False)._from_google_ids(self.ids)
        mapping = {e.google_id: e.id for e in dosyt_events}  # {google_id: dosyt_id}
        existing_google_ids = dosyt_events.mapped('google_id')
        for e in self:
            dosyt_id = mapping.get(e.id)
            if dosyt_id:
                e._events[e.id]['_dosyt_id'] = dosyt_id
    ...
    def clear_type_ambiguity(self, env):
        ...
        recurrences = ambiguous_events._load_dosyt_ids_from_db(env, env['calendar.recurrence'])
    ...
    def exists(self, env) -> 'GoogleEvent':
        ...
        recurrences.dosyt_ids(env)
        events.dosyt_ids(env)

        return self.filter(lambda e: e._dosyt_id)
