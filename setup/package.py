...
exec(open(os.path.join(ROOTDIR, 'dosyt', 'release.py'), 'rb').read())
...
DOCKERUSER = """
RUN mkdir /var/lib/dosyt && \
    groupadd -g %(group_id)s dosyt && \
    useradd -u %(user_id)s -g dosyt dosyt -d /var/lib/dosyt && \
    mkdir /data && \
    chown dosyt:dosyt /var/lib/dosyt /data
USER dosyt
""" % {'group_id': os.getgid(), 'user_id': os.getuid()}


class DosytTestTimeoutError(Exception):
    pass


class DosytTestError(Exception):
    pass
...
def _rpc_count_modules(addr='http://127.0.0.1', port=8069, dbname='mycompany'):
    ...
    if len(modules) > 1:
        ...
        if toinstallmodules:
            ...
            raise DosytTestError("Installation of package failed")
        else:
        ...
        raise DosytTestError("Package test: FAILED. Not able to install base.")
...
def publish(args, pub_type, extensions):
    ...
    for extension in extensions:
        release = glob("%s/dosyt_*.%s" % (args.build_dir, extension))
        if release:
...
def _prepare_build_dir(args, win32=False, move_addons=True):
    ...
    run_cmd(cmd + ['%s/' % args.dosyt_dir, args.build_dir])
    if not move_addons:
        return
    for addon_path in glob(os.path.join(args.build_dir, 'addons/*')):
        if args.blacklist is None or os.path.basename(addon_path) not in args.blacklist:
            try:
                shutil.move(addon_path, os.path.join(args.build_dir, 'dosyt/addons'))
...
class Docker():
    ...
    def __init__(self, args):
        ...
        """
        self.args = args
        self.tag = 'dosyt-%s-%s-nightly-tests' % (DOCKERVERSION, self.arch)
    ...
    def run(self, cmd, build_dir, container_name, user='dosyt', exposed_port=None, detach=False, timeout=None):
    ...
    def test_dosyt(self):
        logging.info('Starting to test Dosyt install test')
        start_time = time.time()
        while self.is_running() and (time.time() - start_time) < INSTALL_TIMEOUT:
            time.sleep(5)
            if os.path.exists(os.path.join(args.build_dir, 'dosyt.pid')):
                try:
        ...
        if self.is_running():
            self.stop()
            raise DosytTestTimeoutError('Dosyt pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise DosytTestError('Error while installing/starting Dosyt after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
...
class DockerTgz(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py sdist --quiet --formats=gztar,zip', self.args.build_dir, 'dosyt-src-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/dosyt-*.tar.gz' % self.args.build_dir)[0], '%s/dosyt_%s.%s.tar.gz' % (self.args.build_dir, VERSION, TSTAMP))
        os.rename(glob('%s/dist/dosyt-*.zip' % self.args.build_dir)[0], '%s/dosyt_%s.%s.zip' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'pip3 install /data/src/dosyt_%s.%s.tar.gz' % (VERSION, TSTAMP),
            'su postgres -s /bin/bash -c "createuser -s dosyt"',
            'su postgres -s /bin/bash -c "createdb mycompany"',
            'su dosyt -s /bin/bash -c "dosyt -d mycompany -i base --stop-after-init"',
            'su dosyt -s /bin/bash -c "dosyt -d mycompany --pidfile=/data/src/dosyt.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'dosyt-src-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_dosyt()
...
class DockerDeb(Docker):
    ...
    def build(self):
        ...
        cmds = ["sed -i '1s/^.*$/dosyt (%s.%s) stable; urgency=low/' debian/changelog" % (VERSION, TSTAMP)]
        ...
        cmds.append('mv ../dosyt_* ./')
        self.run(' && '.join(cmds), self.args.build_dir, 'dosyt-deb-build-%s' % TSTAMP)
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            '/usr/bin/dpkg -i /data/src/dosyt_%s.%s_all.deb ; /usr/bin/apt-get install -f -y' % (VERSION, TSTAMP),
            'su dosyt -s /bin/bash -c "dosyt -d mycompany -i base --stop-after-init"',
            'su dosyt -s /bin/bash -c "dosyt -d mycompany --pidfile=/data/src/dosyt.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'dosyt-deb-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_dosyt()
...
class DockerRpm(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py --quiet bdist_rpm', self.args.build_dir, 'dosyt-rpm-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/dosyt-*.noarch.rpm' % self.args.build_dir)[0], '%s/dosyt_%s.%s.rpm' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'dnf install -d 0 -e 0 /data/src/dosyt_%s.%s.rpm -y' % (VERSION, TSTAMP),
            'su dosyt -s /bin/bash -c "dosyt -c /etc/dosyt/dosyt.conf -d mycompany -i base --stop-after-init"',
            'su dosyt -s /bin/bash -c "dosyt -c /etc/dosyt/dosyt.conf -d mycompany --pidfile=/data/src/dosyt.pid"',
        ]
        self.run(' && '.join(cmds), args.build_dir, 'dosyt-rpm-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_dosyt()
    ...
    def gen_rpm_repo(self, args, rpm_filepath):
        ...
        self.run('createrepo /data/src/', temp_path, 'dosyt-rpm-createrepo-%s' % TSTAMP)
...
class KVMWinBuildExe(KVM):
    def run(self):
        ...
        remote_build_dir = '/cygdrive/c/dosytbuild/server/'

        self.ssh("mkdir -p build")
        logging.info("Syncing Dosyt files to virtual machine...")
...
class KVMWinTestExe(KVM):
    def run(self):
        ...
        setup_path = glob("%s/dosyt_setup_*.exe" % self.args.build_dir)[0]
        setupfile = setup_path.split('/')[-1]
        setupversion = setupfile.split('dosyt_setup_')[1].split('.exe')[0]

        self.rsync(['%s' % setup_path, '%s@127.0.0.1:' % self.login])
        self.ssh("TEMP=/tmp ./%s /S" % setupfile)
        self.ssh('PGPASSWORD=openpgpwd /cygdrive/c/"Program Files"/"Dosyt %s"/PostgreSQL/bin/createdb.exe -e -U openpg mycompany' % setupversion)
        self.ssh('netsh advfirewall set publicprofile state off')
        self.ssh('/cygdrive/c/"Program Files"/"Dosyt {sv}"/python/python.exe \'c:\\Program Files\\Dosyt {sv}\\server\\dosyt-bin\' -d mycompany -i base --stop-after-init'.format(sv=setupversion))
...
def parse_args():
    ...
    # Windows VM
    ap.add_argument("--vm-winxp-image", default='/home/dosyt/vm/win1036/win10_winpy36.qcow2', help="%(default)s")
    ap.add_argument("--vm-winxp-ssh-key", default='/home/dosyt/vm/win1036/id_rsa', help="%(default)s")
