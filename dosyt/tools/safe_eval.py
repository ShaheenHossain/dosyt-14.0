...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False):
    ...
    except dosyt.exceptions.UserError:
        raise
    except dosyt.exceptions.RedirectWarning:
        raise
    except werkzeug.exceptions.HTTPException:
        raise
    except dosyt.http.AuthenticationError:
        raise
...
`dosyt.tools.safe_eval.wrap_module` to generate a wrapper recursively
whitelisting allowed attributes.

Pre-wrapped modules are provided as attributes of `dosyt.tools.safe_eval`.
""")
    return d
