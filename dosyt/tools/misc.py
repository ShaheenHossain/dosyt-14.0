...
from dosyt.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if dosyt.tools.config['db_host']:
        env['PGHOST'] = dosyt.tools.config['db_host']
    if dosyt.tools.config['db_port']:
        env['PGPORT'] = str(dosyt.tools.config['db_port'])
    if dosyt.tools.config['db_user']:
        env['PGUSER'] = dosyt.tools.config['db_user']
    if dosyt.tools.config['db_password']:
        env['PGPASSWORD'] = dosyt.tools.config['db_password']
    return env
...
def _fileopen(path, mode, basedir, pathinfo, basename=None, filter_ext=None, env=None):
    ...
    paths = dosyt.addons.__path__ + [config['root_path']]
    if env and hasattr(env.all, '__file_open_tmp_paths'):
...
def file_open_temporary_directory(env):
    ...
    Examples::
        ...
        >>> with dosyt.tools.file_open_temporary_directory(self.env) as module_dir:
        ...    with zipfile.ZipFile('foo.zip', 'r') as z:
        ...        z.extract('foo/__manifest__.py', module_dir)
        ...    with dosyt.tools.file_open('foo/__manifest__.py', env=self.env) as f:
        ...        manifest = f.read()
...
def scan_languages():
    ...
    :rtype: [(str, unicode)]
    """
    csvpath = dosyt.modules.module.get_resource_path('base', 'data', 'res.lang.csv')
    try:
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('dosyt.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('dosyt.foo.bar'):
            do_suff()
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = dosyt.fields.Datetime.from_string(value)
            value = dosyt.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = dosyt.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = dosyt.fields.Datetime.from_string(value)
    else:
        timestamp = value
