...
import dosyt
from dosyt.tools.config import config
...
class Cloc(object):
    ...
    def count_path(self, path, exclude=None):
        path = path.rstrip('/')
        exclude_list = []
        for i in dosyt.modules.module.MANIFEST_NAMES:
    ...
    def count_modules(self, env):
        ...
        exclude_heuristic = [dosyt.modules.get_module_path(m, display_warning=False) for m in STANDARD_MODULES]
        ...
        for module_name in module_list:
            module_path = os.path.realpath(dosyt.modules.get_module_path(module_name))
            if module_path:
    ...
    def count_customization(self, env):
        ...
        # the module should be dosyt/studio unless it comes from an imported module install
    ...
    def count_database(self, database):
        with dosyt.api.Environment.manage():
            registry = dosyt.registry(config['db_name'])
            with registry.cursor() as cr:
                uid = dosyt.SUPERUSER_ID
                env = dosyt.api.Environment(cr, uid, {})
                self.count_env(env)
