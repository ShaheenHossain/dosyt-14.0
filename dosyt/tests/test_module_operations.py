...
import dosyt
from dosyt.tools import config, topological_sort, unique
from dosyt.netsvc import init_logger
from dosyt.tests import standalone_tests
import dosyt.tests.loader

_logger = logging.getLogger('dosyt.tests.test_module_operations')
...
def install(db_name, module_id, module_name):
    with dosyt.api.Environment.manage():
        with dosyt.registry(db_name).cursor() as cr:
            env = dosyt.api.Environment(cr, dosyt.SUPERUSER_ID, {})
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall cycle of Dosyt modules")
    ...
    parser.add_argument("--addons-path", "-p", type=str, action=CheckAddons,
        help="Comma-separated list of paths to directories containing extra Dosyt modules")
...
def test_full(args):
    ...
    with dosyt.api.Environment.manage():
        with dosyt.registry(args.database).cursor() as cr:
            env = dosyt.api.Environment(cr, dosyt.SUPERUSER_ID, {})
...
def test_scripts(args):
    ...
    registry = dosyt.registry(args.database)
    for module_name in registry._init_modules:
        # import tests for loaded modules
        dosyt.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        dosyt.tools.config['addons_path'] = ','.join([args.addons_path, dosyt.tools.config['addons_path']])
        if args.data_dir:
            dosyt.tools.config['data_dir'] = args.data_dir
        dosyt.modules.module.initialize_sys_path()

    init_logger()
    logging.getLogger('dosyt.modules.loading').setLevel(logging.CRITICAL)
    logging.getLogger('dosyt.sql_db').setLevel(logging.CRITICAL)
