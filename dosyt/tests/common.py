# -*- coding: utf-8 -*-
"""
The module :mod:`dosyt.tests.common` provides unittest test cases and a few
...
from dosyt.models import BaseModel
from dosyt.osv.expression import normalize_domain, TRUE_LEAF, FALSE_LEAF
from dosyt.service import security
from dosyt.sql_db import Cursor
from dosyt.tools import float_compare, single_email_re
from dosyt.tools.misc import find_in_path
from dosyt.tools.safe_eval import safe_eval
...
# The dosyt library is supposed already configured.
ADDONS_PATH = dosyt.tools.config['addons_path']
HOST = '127.0.0.1'
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = dosyt.SUPERUSER_ID


def get_db_name():
    db = dosyt.tools.config['db_name']
...
def standalone(*tags):
    ...
    """
    def register(func):
        # register func by dosyt module name
        if func.__module__.startswith('dosyt.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most dosyt operations;
    """
    if not login:
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('dosyt.addons.'):
...
class BaseCase(TreeCase, MetaCase('DummyCase', (object,), {})):
    ...
    longMessage = True      # more verbose error message by default: https://www.dosyt.com/r/Vmh
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~dosyt.models.BaseModel`
        """
    ...
    def assertQueries(self, expected, flush=True):
        ...
        with patch('dosyt.sql_db.Cursor.execute', execute):
            with patch('dosyt.osv.expression.get_unaccent_wrapper', get_unaccent_wrapper):
        ...
        if self.warm:
            ...
            with patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/dosyt/addons/" in filename:
                        filename = filename.rsplit("/dosyt/addons/", 1)[1]
                    if count > expected:
    ...
    def _callSetUp(self):
        ...
        # See https://github.com/dosyt/dosyt/pull/107572 for more info.
...
class _ErrorCatcher(list):
    ...
    def _complete_traceback(self, initial_tb):
        ...
        # remove traceback root part (dosyt_bin, main, loading, ...), as
...
class TransactionCase(BaseCase):
    ...
    def setUp(self):
        ...
        self.registry = dosyt.registry(get_db_name())
        ...
        #: :class:`~dosyt.api.Environment` for the current test case
        self.env = api.Environment(self.cr, dosyt.SUPERUSER_ID, {})
...
class SingleTransactionCase(BaseCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.registry = dosyt.registry(get_db_name())
        ...
        cls.env = api.Environment(cls.cr, dosyt.SUPERUSER_ID, {})
...
class ChromeBrowser():
    ...
    def __init__(self, logger, window_size, test_class):
        ...
        self.request_id = 0
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_dosyt')
        self.chrome_pid = None

        otc = dosyt.tools.config
...
class HttpCaseCommon(BaseCase):
    ...
    def __init__(self, methodName='runTest'):
        ...
        self.xmlrpc_url = url_8 = 'http://%s:%d/xmlrpc/2/' % (HOST, dosyt.tools.config['http_port'])
    ...
    def url_open(self, url, data=None, files=None, timeout=10, headers=None, allow_redirects=True):
        self.env['base'].flush()
        if url.startswith('/'):
            url = "http://%s:%s%s" % (HOST, dosyt.tools.config['http_port'], url)
        if data or files:
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('dosyt.service.http.request.')]
        ...
        if request_threads:
            ...
            dosyt.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=True)
        dosyt.http.root.session_store.save(self.session)

    def authenticate(self, user, password):
        db = get_db_name()
        if getattr(self, 'session', None):
            dosyt.http.root.session_store.delete(self.session)

        self.session = session  = dosyt.http.root.session_store.new()
        session.db = db
        ...
        dosyt.http.root.session_store.save(session)
    ...
    def browser_js(self, url_path, code, ready='', login=None, timeout=60, **kw):
        ...
        try:
            ...
            base_url = "http://%s:%s" % (HOST, dosyt.tools.config['http_port'])
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', "dosyt.startTour('%s'%s)" % (tour_name, step_delay))
        ready = kwargs.pop('ready', "dosyt.__DEBUG__.services['web_tour.tour'].tours['%s'].ready" % tour_name)
...
class Form(object):
    ...
    * properly handle defaults & onchanges around x2many fields
    ...
    :class:`~dosyt.fields.Many2one` fields assign a singleton recordset::
    ...
    For :class:`~dosyt.fields.Many2many` fields, the field itself is a
    :class:`~dosyt.tests.common.M2MProxy` and can be altered by adding or
    removing records::
    ...
    Finally :class:`~dosyt.fields.One2many` are reified as
    :class:`~dosyt.tests.common.O2MProxy`.

    Because the :class:`~dosyt.fields.One2many` only exists through its
    parent, it is manipulated more directly by creating "sub-forms"
    with the :meth:`~dosyt.tests.common.O2MProxy.new` and
    :meth:`~dosyt.tests.common.O2MProxy.edit` methods. These would
    ...
    :type recordp: dosyt.models.Model
    ...
    :type view: int | str | dosyt.model.Model
...
class O2MProxy(X2MProxy):
    ...
    def new(self):
        ...
        :class:`~dosyt.fields.One2many` record, properly initialised.
    ...
    def edit(self, index):
        ...
        :class:`~dosyt.fields.One2many` record.
...
def record_to_values(fields, record):
    ...
    for f, v in record.read(to_read)[0].items():
        ...
        if descr['type'] == 'many2one':
            ...
            v = dosyt.fields.Datetime.to_string(v)
        elif descr['type'] == 'date' and isinstance(v, date):
            v = dosyt.fields.Date.to_string(v)
        r[f] = v
    return r
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return dosyt.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return dosyt.fields.Date.to_string(value)
    return value
...
def tagged(*tags):
    ...
    By default, all Test classes from dosyt.tests.common have a test_tags
