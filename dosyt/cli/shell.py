...
class Shell(Command):
    """Start dosyt in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parse_config(args)
        dosyt.cli.server.report_configuration()
        dosyt.service.server.start(preload=[], stop=True)
        ...
        with dosyt.api.Environment.manage():
            if dbname:
                ...
                with registry.cursor() as cr:
                    ...
                    ctx = dosyt.api.Environment(cr, uid, {})['res.users'].context_get()
                    env = dosyt.api.Environment(cr, uid, ctx)
                    local_vars['env'] = env
