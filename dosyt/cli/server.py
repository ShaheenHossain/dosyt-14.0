...
__author__ = dosyt.release.author
__version__ = dosyt.release.version

# Also use the `dosyt` logger for the main script.
_logger = logging.getLogger('dosyt')
...
def report_configuration():
    ...
    _logger.info('addons paths: %s', dosyt.addons.__path__)
...
def setup_pid_file():
    ...
    """
    config = dosyt.tools.config
    if not dosyt.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = dosyt.modules.registry.Registry.new(dbname)
        with dosyt.api.Environment.manage():
            with registry.cursor() as cr:
                dosyt.tools.trans_export(config["language"],
...
def import_translation():
    ...
    with dosyt.api.Environment.manage():
        with registry.cursor() as cr:
            dosyt.tools.trans_load(
                cr, config["translate_in"], config["language"], overwrite=overwrite,
            )

def main(args):
    check_root_user()
    dosyt.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                dosyt.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except dosyt.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        dosyt.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = dosyt.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the dosyt server (default command)"""
    def run(self, args):
        main(args)
