...
from dosyt.exceptions import AccessDenied
import dosyt.release
import dosyt.sql_db
import dosyt.tools
from dosyt.sql_db import db_connect
from dosyt.release import version_info
...
def check_db_management_enabled(method):
    def if_db_mgt_enabled(method, self, *args, **kwargs):
        if not dosyt.tools.config['list_db']:
...
def check_super(passwd):
    if passwd and dosyt.tools.config.verify_admin_password(passwd):
        return True
    raise dosyt.exceptions.AccessDenied()

# This should be moved to dosyt.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        db = dosyt.sql_db.db_connect(db_name)
        with closing(db.cursor()) as cr:
            # TODO this should be removed as it is done by Registry.new().
            dosyt.modules.db.initialize(cr)
            dosyt.tools.config['load_language'] = lang
            cr.commit()

        registry = dosyt.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(db.cursor()) as cr:
            env = dosyt.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = dosyt.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = dosyt.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = dosyt.tools.config['db_template']
    ...
    if dosyt.tools.config['unaccent']:
        try:
            db = dosyt.sql_db.db_connect(name)
            with closing(db.cursor()) as cr:
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    dosyt.sql_db.close_db(db_original_name)
    db = dosyt.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        cr.autocommit(True)     # avoid transaction block
    ...
    registry = dosyt.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = dosyt.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)

    from_fs = dosyt.tools.config.filestore(db_original_name)
    to_fs = dosyt.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    dosyt.modules.registry.Registry.delete(db_name)
    dosyt.sql_db.close_db(db_name)
    ...
    fs = dosyt.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'dosyt_dump': '1',
        'db_name': cr.dbname,
        'version': dosyt.release.version,
        'version_info': dosyt.release.version_info,
        'major_version': dosyt.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with tempfile.TemporaryDirectory() as dump_dir:
            filestore = dosyt.tools.config.filestore(db_name)
            ...
            dosyt.tools.exec_pg_command(*cmd)
            if stream:
                dosyt.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                dosyt.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
        cmd.insert(-1, '--format=c')
        stdin, stdout = dosyt.tools.exec_pg_command_pipe(*cmd)
        if stream:
...
def restore_db(db, dump_file, copy=False):
    ...
    with tempfile.TemporaryDirectory() as dump_dir:
        ...
        if dosyt.tools.exec_pg_command(pg_cmd, *pg_args):
            raise Exception("Couldn't restore database")

        registry = dosyt.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = dosyt.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    dosyt.modules.registry.Registry.delete(old_name)
    dosyt.sql_db.close_db(old_name)
    ...
    old_fs = dosyt.tools.config.filestore(old_name)
    new_fs = dosyt.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    dosyt.tools.config.set_admin_password(new_password)
    dosyt.tools.config.save()
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        dosyt.tools.config['update']['base'] = True
        dosyt.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@dosyt.tools.mute_logger('dosyt.sql_db')
...
def list_dbs(force=False):
    if not dosyt.tools.config['list_db'] and not force:
        raise dosyt.exceptions.AccessDenied()

    if not dosyt.tools.config['dbfilter'] and dosyt.tools.config['db_name']:
        ...
        res = sorted(db.strip() for db in dosyt.tools.config['db_name'].split(','))
        return res
    ...
    with closing(db.cursor()) as cr:
        try:
            ...
            res = [dosyt.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
...
def list_db_incompatible(databases):
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if dosyt.tools.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        dosyt.sql_db.close_db(database_name)
...
def exp_list_lang():
    return dosyt.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(dosyt.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()
