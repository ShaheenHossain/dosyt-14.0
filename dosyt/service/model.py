...
from dosyt.tools.translate import translate, translate_sql_constraint
...
def dispatch(method, params):
    ...
    registry = dosyt.registry(db).check_signaling()
    fn = globals()[method]
...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
        ...
        def tr(src, ttype):
            ...
            with closing(dosyt.sql_db.db_connect(dbname).cursor()) as cr:
        ...
        while True:
            try:
                if dosyt.registry(dbname)._init and not dosyt.tools.config['test_enable']:
                    raise dosyt.exceptions.Warning('Currently, this database is not fully loaded and can not be used.')
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    dosyt.api.Environment.reset()  # clean cache etc if we retry the same transaction
    recs = dosyt.api.Environment(cr, uid, {}).get(obj)
    if recs is None:
        raise UserError(_("Object %s doesn't exist", obj))
    result = dosyt.api.call_kw(recs, method, args, kw)
...
def execute(db, uid, obj, method, *args, **kw):
    ...
    with dosyt.registry(db).cursor() as cr:
