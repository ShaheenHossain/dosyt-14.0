...
# *broken*
NO_POSTMORTEM = (dosyt.exceptions.AccessDenied,
                 dosyt.exceptions.UserError,
                 dosyt.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                dosyt.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = dosyt.service.common.dispatch
        elif service_name == 'db':
            dispatch = dosyt.service.db.dispatch
        elif service_name == 'object':
            dispatch = dosyt.service.model.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                dosyt.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                dosyt.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except Exception as e:
        _logger.exception(dosyt.tools.exception_to_unicode(e))
        dosyt.tools.debugger.post_mortem(dosyt.tools.config, sys.exc_info())
        raise
...
class WebRequest(object):
    ...
    def __init__(self, httprequest):
        ...
        # dispatching phase in dosyt.service.wsgi_server.application
        if self.db:
    ...
    @property
    def cr(self):
        """ :class:`~dosyt.sql_db.Cursor` initialized for the current method call.
    ...
    @property
    def env(self):
        """ The :class:`~dosyt.api.Environment` bound to current request. """
        if self._env is None:
            self._env = dosyt.api.Environment(self.cr, self.uid, self.context)
        return self._env
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            dosyt.tools.debugger.post_mortem(
                dosyt.tools.config, sys.exc_info())
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <dosyt.http.WebRequest.csrf_token`, the
          :data:`~dosyt.http.request` object is available by default
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, SessionExpiredException):
                ...
                elif isinstance(exception, (dosyt.exceptions.UserError,
...
HTTP methods). See
https://www.dosyt.com/documentation/14.0/developer/reference/http.html#csrf for
more details.
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["dosyt", "addons"]:
            module = ""
        else:
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('dosyt.addons.') and c.__module__.split(".")[2] in modules
...
class OpenERPSession(sessions.Session):
    ...
    def authenticate(self, db, login=None, password=None):
        ...
        uid = dosyt.registry(db)['res.users'].authenticate(db, login, password, env)
        self.pre_uid = uid
    ...
    def check_security(self):
        ...
        env = dosyt.api.Environment(request.cr, self.uid, self.context)
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        """
        env = request.env(user=self.uid or request.uid or dosyt.SUPERUSER_ID)
...
class Root(object):
    ...
    def session_store(self):
        # Setup http sessions
        path = dosyt.tools.config.session_dir
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint, routing in dosyt.http._generate_routing_rules([''] + dosyt.conf.server_wide_modules, True):
    ...
    def load_addons(self):
        ...
        statics = {}
        for addons_path in dosyt.addons.__path__:
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request:
                ...
                if db:
                    try:
                        dosyt.registry(db).check_signaling()
                        with dosyt.tools.mute_logger('dosyt.sql_db'):
...
def db_list(force=False, httprequest=None):
    dbs = dosyt.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    if dosyt.tools.config['dbfilter']:
        d, h = re.escape(d), re.escape(h)
        r = dosyt.tools.config['dbfilter'].replace('%h', h).replace('%d', d)
        dbs = [i for i in dbs if re.match(r, i)]
    elif dosyt.tools.config['db_name']:
        ...
        exposed_dbs = set(db.strip() for db in dosyt.tools.config['db_name'].split(','))
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = dosyt.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
def content_disposition(filename):
    filename = dosyt.tools.ustr(filename)
