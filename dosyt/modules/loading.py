...
import dosyt
import dosyt.modules.db
import dosyt.modules.graph
import dosyt.modules.migration
...
_test_logger = logging.getLogger('dosyt.tests')
...
def force_demo(cr):
    ...
    """
    graph = dosyt.modules.graph.Graph()
...
def load_module_graph(cr, graph, status=None, perform_checks=True,
    ...
    migrations = dosyt.modules.migration.MigrationManager(cr, graph)
    module_count = len(graph)
    _logger.info('loading %d modules...', module_count)

    # register, instantiate and initialize models for each modules
    t0 = time.time()
    loading_extra_query_count = dosyt.sql_db.sql_counter
    ...
    for index, package in enumerate(graph, 1):
        ...
        module_extra_query_count = dosyt.sql_db.sql_counter
        ...
        if new_install:
            py_module = sys.modules['dosyt.addons.%s' % (module_name,)]
        ...
        if needs_update:
            ...
            overwrite = dosyt.tools.config["overwrite_existing_translations"]
        ...
        if tools.config.options['test_enable'] and (needs_update or not updating):
            ...
            if suite.countTestCases():
                ...
                tests_t0, tests_q0 = time.time(), dosyt.sql_db.sql_counter
                test_results = loader.run_suite(suite, module_name)
                report.update(test_results)
                test_time = time.time() - tests_t0
                test_queries = dosyt.sql_db.sql_counter - tests_q0
        ...
        extra_queries = dosyt.sql_db.sql_counter - module_extra_query_count - test_queries
        extras = []
        if test_queries:
    ...
    _logger.runbot("%s modules loaded in %.2fs, %s queries (+%s extra)",
                   ...
                   dosyt.sql_db.sql_counter - loading_extra_query_count)  # extra queries: testes, notify, any other closed cursor
...
def load_modules(db, force_demo=False, status=None, update_module=False):
    ...
    with db.cursor() as cr:
        ...
        if not dosyt.modules.db.is_initialized(cr):
        ...
        # dosyt.modules.registry.Registry.new().
        registry = dosyt.registry(cr.dbname)
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['dosyt.addons.%s' % (pkg.name,)]
                ...
                registry = dosyt.modules.registry.Registry.new(
