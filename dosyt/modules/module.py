...
import dosyt
import dosyt.tools as tools
import dosyt.release as release
from dosyt.tools import pycompat
...
# ad_paths is a deprecated alias, please use dosyt.addons.__path__
@tools.lazy
def ad_paths():
    warnings.warn(
        '"dosyt.modules.module.ad_paths" is a deprecated proxy to '
        '"dosyt.addons.__path__".', DeprecationWarning, stacklevel=2)
    return dosyt.addons.__path__
...
class AddonsHook(object):
    ...
    def find_module(self, name, path=None):
        if name.startswith('openerp.addons.') and name.count('.') == 2:
            warnings.warn(
                '"openerp.addons" is a deprecated alias to "dosyt.addons".',
    ...
    def load_module(self, name):
        ...
        dosyt_name = re.sub(r'^openerp.addons.(\w+)$', r'dosyt.addons.\g<1>', name)

        dosyt_module = sys.modules.get(dosyt_name)
        if not dosyt_module:
            dosyt_module = importlib.import_module(dosyt_name)

        sys.modules[name] = dosyt_module

        return dosyt_module

class DosytHook(object):
    """ Makes dosyt package also available as openerp """
    ...
    def find_module(self, name, path=None):
        ...
        if re.match(r'^openerp\b', name):
            warnings.warn(
                'openerp is a deprecated alias to dosyt.',
                DeprecationWarning, stacklevel=2)
            return self
    ...
    def load_module(self, name):
        ...
        canonical = re.sub(r'^openerp(.*)', r'dosyt\g<1>', name)
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `dosyt.upgrade`"""

    def find_module(self, name, path=None):
        if re.match(r"^dosyt.addons.base.maintenance.migrations\b", name):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("dosyt.addons.base.maintenance.migrations", "dosyt.upgrade")
...
def initialize_sys_path():
    """
    Setup the addons path ``dosyt.addons.__path__`` with various defaults
    and explicit directories.
    """
    # hook dosyt.addons on data dir
    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in dosyt.addons.__path__:
        dosyt.addons.__path__.append(dd)

    # hook dosyt.addons on addons paths
    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(tools.ustr(ad.strip())))
        if ad not in dosyt.addons.__path__:
            dosyt.addons.__path__.append(ad)

    # hook dosyt.addons on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'addons')))
    if base_path not in dosyt.addons.__path__ and os.path.isdir(base_path):
        dosyt.addons.__path__.append(base_path)

    # hook dosyt.upgrade on upgrade-path
    from dosyt import upgrade
    ...
    # create decrecated module alias from dosyt.addons.base.maintenance.migrations to dosyt.upgrade
    spec = importlib.machinery.ModuleSpec("dosyt.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["dosyt.addons.base.maintenance"] = maintenance_pkg
    sys.modules["dosyt.addons.base.maintenance.migrations"] = upgrade

    # hook deprecated module alias from openerp to dosyt and "crm"-like to dosyt.addons
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    """
    for adp in dosyt.addons.__path__:
...
def get_module_filetree(module, dir='.'):
    ...
    files = dosyt.tools.osutil.listdir(path, True)
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    for adpath in dosyt.addons.__path__:
...
def load_openerp_module(module_name):
    ...
    try:
        __import__('dosyt.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['dosyt.addons.' + module_name], info['post_load'])()
...
def get_modules():
    ...
    plist = []
    for ad in dosyt.addons.__path__:
