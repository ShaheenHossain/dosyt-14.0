...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with tools.ignore(Exception), tools.mute_logger('dosyt.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', r'^Sampling from a set', category=DeprecationWarning, module='dosyt')
...
DEFAULT_LOG_CONFIGURATION = [
    'dosyt.http.rpc.request:INFO',
    'dosyt.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['dosyt:DEBUG', 'dosyt.sql_db:INFO', 'dosyt.http.rpc:DEBUG'],
    'debug_rpc': ['dosyt:DEBUG', 'dosyt.sql_db:INFO', 'dosyt.http.rpc.request:DEBUG'],
    'debug': ['dosyt:DEBUG', 'dosyt.sql_db:INFO'],
    'debug_sql': ['dosyt.sql_db:DEBUG'],
    'info': [],
    'runbot': ['dosyt:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['dosyt:WARNING', 'werkzeug:WARNING'],
    'error': ['dosyt:ERROR', 'werkzeug:ERROR'],
    'critical': ['dosyt:CRITICAL', 'werkzeug:CRITICAL'],
}
