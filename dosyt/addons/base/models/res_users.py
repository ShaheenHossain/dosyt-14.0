...
from dosyt import api, fields, models, tools, SUPERUSER_ID, _
from dosyt.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
from dosyt.exceptions import AccessDenied, AccessError, UserError, ValidationError
from dosyt.http import request
from dosyt.modules.module import get_module_resource
from dosyt.osv import expression
from dosyt.service.db import check_super
from dosyt.tools import partition, collections, frozendict, lazy_property, image_process
...
class Users(models.Model):
    ...
    def _get_default_image(self):
        """ Get a default image when the user is created without image
            ...
            https://github.com/dosyt/dosyt/blob/11.0/dosyt/addons/base/res/res_partner.py
        """
    ...
    def unlink(self):
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by Dosyt (updates, module installation, ...)'))
        self.clear_caches()
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: dosyt.exceptions.AccessDenied when old password is wrong
        :raise: dosyt.exceptions.UserError when new password is not set or empty
        """
        if not old_passwd:
            raise AccessDenied()
        if not new_passwd:
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warning(
                    ...
                    "and *might* be a proxy. If your Dosyt is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "Dosyt in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.dosyt.com/documentation/14.0/administration/deployment/deploy.html#https for details.",
                    source
                )
