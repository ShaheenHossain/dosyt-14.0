...
from dosyt.exceptions import UserError, AccessError
from dosyt.tools.safe_eval import safe_eval, time
from dosyt.tools.misc import find_in_path, ustr
from dosyt.tools import config
from dosyt.sql_db import TestCursor
from dosyt.http import request
from dosyt.osv.expression import NEGATIVE_TERM_OPERATORS, FALSE_DOMAIN
...
# here to init the T1 fonts cache at the start-up of Dosyt so that rendering of barcode in multiple
...
# Check the presence of Wkhtmltopdf and return its version at Dosyt start-up
...
try:
    ...
    if match:
        ...
        if config['workers'] == 1:
            _logger.info('You need to start Dosyt with at least two workers to print a pdf version of the reports.')
...
class IrActionsReport(models.Model):
    ...
    def _raise_on_unreadable_pdfs(self, streams, stream_record):
        ...
        if unreadable_pdfs:
            ...
            raise UserError(_(
                "Dosyt is unable to merge the PDFs attached to the following records:\n"
                "%s\n\n"
