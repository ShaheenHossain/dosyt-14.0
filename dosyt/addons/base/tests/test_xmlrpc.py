...
from dosyt.exceptions import AccessDenied, AccessError
from dosyt.http import _request_stack

import dosyt.tools
from dosyt.tests import common
from dosyt.service import common as auth, model
...
class TestXMLRPC(common.HttpCase):
    ...
    def _json_call(self, *args):
        self.opener.post("http://%s:%s/jsonrpc" % (common.HOST, dosyt.tools.config['http_port']), json={
            'jsonrpc': '2.0',
            'id': None,
            'method': 'call',
            'params': {
