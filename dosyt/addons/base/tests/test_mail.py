...
from dosyt.tests.common import BaseCase, SavepointCase, TransactionCase
from dosyt.tools import (
...
class EmailConfigCase(SavepointCase):
    @patch.dict("dosyt.tools.config.options", {"email_from": "settings@example.com"})
    ...
    def test_email_from_rewrite(self):
        ...
        set_param('mail.catchall.domain', 'dosyt.example.com')
        ...
        set_param('mail.force.smtp.from', False)
        set_param('mail.dynamic.smtp.from', 'notification@dosyt.example.com')
        set_param('mail.catchall.domain', 'dosyt.example.com')

        email_from, return_path = get_email_from('"Admin" <admin@test.example.com>')
        self.assertEqual(email_from, '"Admin" <notification@dosyt.example.com>',
                         msg='Domain is not the same as the catchall domain, we should force the email FROM')
        self.assertEqual(return_path, 'notification@dosyt.example.com')

        email_from, return_path = get_email_from('"Admin" <admin@dosyt.example.com>')
        self.assertEqual(email_from, '"Admin" <admin@dosyt.example.com>',
        ...
        set_param('mail.force.smtp.from', False)
        set_param('mail.dynamic.smtp.from', '"Your notification bot" <notification@dosyt.example.com>')
        set_param('mail.catchall.domain', 'dosyt.example.com')
